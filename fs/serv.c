...
serve_read(envid_t envid, union Fsipc *ipc)
{
	...
	if((r = openfile_lookup(0, req->req_fileid, &po)) < 0)
		return r;
	ssize_t read_count = file_read(po->o_file, (void*)ret->ret_buf, req->req_n, po->o_fd->fd_offset);
	if(read_count > 0)
		po->o_fd->fd_offset += read_count;
	return read_count;
}
...
serve_write(envid_t envid, struct Fsreq_write *req)
{
	...
	if((r = openfile_lookup(0, req->req_fileid, &po)) < 0)
		return r;
	int write_count = file_write(po->o_file, req->req_buf, req->req_n, po->o_fd->fd_offset);
	if(write_count > 0)
		po->o_fd->fd_offset += write_count;
	return write_count;
