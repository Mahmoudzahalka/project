...
int sys_net_try_send(char* buffer, uint32_t size);
int sys_net_try_receive(char* buffer, uint32_t size);
int sys_get_mac_address(uint32_t *address);
int sys_env_copy_fd(envid_t envid, uint32_t fd);
int sys_get_tail_idx(void);
int sys_add_descriptor_buffers(void);
int sys_fs_clear_access_bit(uint32_t addr);
...
//lab 6
ssize_t	read_nowait(int fd, void *buf, size_t nbytes);
