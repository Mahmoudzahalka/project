...
static ssize_t devsock_read_nowait(struct Fd *fd, void *buf, size_t n);
...
struct Dev devsock =
{
	...
	.dev_read_nowait = devsock_read_nowait
};
...
static ssize_t
devsock_read_nowait(struct Fd *fd, void *buf, size_t n)
{
	return nsipc_recv(fd->fd_sock.sockid, buf, n, MSG_DONTWAIT);
}
